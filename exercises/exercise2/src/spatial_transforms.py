...
class FiveCrops(object):
    ...
    def __call__(self, img, inv, flow):
        ...
        crop_positions += [[x1, y1, x2, y2]]
        cropped_imgs = [
            img.crop(crop_positions[i]).resize(
        ...
        if self.tenCrops is True:
            if inv is True:
                flipped_imgs = [
                    ImageOps.invert(cropped_imgs[i].transpose(Image.FLIP_LEFT_RIGHT))
                    for i in range(5)
                ]
            else:
                flipped_imgs = [
                    cropped_imgs[i].transpose(Image.FLIP_LEFT_RIGHT) for i in range(5)
                ]
        ...
        if not self.full224:
            ...
            for i, x in enumerate(pos_x):
                ...
                for y in pos_y:
                    ...
                    if not self.regression:
                        value = round(
                            int(tensor[start_x:end_x, start_y:end_y].sum())
                            / ((2 * K + 1) ** 2),
                            0,
                        )
                    else:
                        value = tensor[start_x:end_x, start_y:end_y].sum() / (
                            (2 * K + 1) ** 2
                        )
            ...
            for pos, i in enumerate(jumps):
                ...
                for j in jumps:
                    ...
                    if not self.regression:
                        value = round(
                            int(tensor[i : i + step, j : j + step].sum()) / (step**2), 0
                        )
                    else:
