...
from src.spatial_transforms import ToTensor
...
class GTEA61(VisionDataset):
    ...
    def __init__(
        ...
        label_map=None,
        mmaps=False,
        mmaps_transform=None,
        static_frames=False,
    ):
        super(GTEA61, self).__init__(
            root, transform=transform, target_transform=target_transform
        )
        self.datadir = root  # Directory of the dataset
        self.split = split  # train or test
        self.get_mmaps = mmaps  # if we want to load also the mmaps
        self.seq_len = seq_len  # seq len tells us how many uniform spaced frames are going to be considered for each video
        self.label_map = label_map  # label map is a dictionary that maps the class names to class IDs
        self.mmaps = mmaps  # if we want to load also the mmaps
        self.mmaps_transform = mmaps_transform  # the transformation for the mmaps
        self.static_frames = static_frames  # if we want to use static frames

        # if the label map dictionary is not provided, build it
        if label_map is None:
            self.label_map = {}

        # videos[i] contains the path of the i-th video frames
        self.videos = []
        ...
        if not self.has_to_tensor:
            raise ValueError(
                "you did NOT provide ToTensor as a transformation for rgbs"
            )

        # If we want to load also the mmaps, we check if ToTensor is among the transformations
        if mmaps:
            check_mmaps_totensor = [
                isinstance(tr, ToTensor) for tr in self.mmaps_transform.transforms
            ]
            self.mmaps_has_to_tensor = True in check_mmaps_totensor
            if not self.mmaps_has_to_tensor:
                raise ValueError(
                    "you did NOT provide ToTensor as a transformation for mmaps"
                )
        ...
        # Build label map from the name of folders
        if label_map is None:
        ...
        # Fill the lists videos, labels, n_frames, and mmaps
    ...
    def __getitem__(self, index):
        # Retrieve the video path, label and num of frames
        vid = self.videos[index]
        label = self.labels[index]
        length = self.n_frames[index]

        if self.transform is not None:
            # Needed to randomize the parameters of the random transformations
        ...
        # sort the list of frames since the name is like rgb002.png, last number as ordering
        ...
        # select the frames
        # note: numpy arrays are arrays of strings, containing the file names. Nevertheless, numpy will work with string arrays as well.
        select_frames = frames[select_indices]

        # path to each selected image
        select_files = [os.path.join(vid, frame) for frame in select_frames]

        # Load images: use pil_loader to get pil objects
        sequence = [pil_loader(file) for file in select_files]
        if self.get_mmaps:
            # replace folder
            select_map = [
                os.path.join(
                    os.path.dirname(file).replace(RGB_FOLDER, MMAPS_FOLDER),
                    os.path.basename(file).replace(RGB_FILENAME, MMAP_FILENAME),
                )
        ...
        if not self.static_frames:
            # No static frames: Uniform sampling
            if self.transform is not None:
                ...
                if self.get_mmaps:
                    maps_sequence = [
                        self.mmaps_transform(mmap) for mmap in maps_sequence
                    ]
                    if self.has_to_tensor:
...
class GTEA61_flow(VisionDataset):
    ...
    def __init__(
        ...
        super(GTEA61_flow, self).__init__(
            root, transform=transform, target_transform=target_transform
        )
    ...
    def get_selected_files(self, vid_x, frames_x, frames_y, select_indices):
        ...
        select_files[1::2] = [
            y_files.replace("x", "y") for y_files in select_files[1::2]
        ]
        # create pil objects
        ...
        if self.transform is not None:
            ...
            sequence[::2] = [
                self.transform(image, inv=True, flow=True) for image in sequence[::2]
            ]
            sequence[1::2] = [
                self.transform(image, inv=False, flow=True) for image in sequence[1::2]
            ]
    ...
    def __getitem__(self, index):
        ...
        if self.n_seq > 0:
            segments = []
            starting_frames = np.linspace(
                1, length - self.seq_len + 1, self.n_seq, endpoint=False, dtype=int
            )
            for start_frame in starting_frames:
                select_indices = start_frame + np.arange(0, self.seq_len)
                sequence = self.get_selected_files(
                    vid_x, frames_x, frames_y, select_indices
                )
...
class GTEA61_2Stream(VisionDataset):
    ...
    def __init__(
        ...
        super(GTEA61_2Stream, self).__init__(
            root, transform=transform, target_transform=target_transform
        )
        # we expect datadir to be ../GTEA61
        self.datadir = root
        ...
        self.flow_dataset = GTEA61_flow(
            self.datadir,
            split=self.split,
            seq_len=self.stack_size,
            transform=self.transform,
            label_map=self.label_map,
        )
